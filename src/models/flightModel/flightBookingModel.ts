...
import { ICreateFlightBookingPayload, IInsertFlightSegmentPayload, IInsertFlightTravelerPayload } from '../../utils/interfaces/flight/flightBookingInterface';
...
class FlightBookingModel extends Schema {
  ...
  public async getAllFlightBooking(payload: {
    ...
    const data = await this.db('booking.flight_booking as fb')
      .select(
        'fb.id as booking_id',
      ...
      .leftJoin('user.users as us', 'us.id', 'fb.created_by')
      .where(function () {
        if (user_id) {
        ...
        if (filter) {
          this.andWhere(function () {
            this.where('us.username', 'ilike', `%${filter}%`)
    ...
    const total = await this.db('booking.flight_booking as fb')
      .count(
        'fb.id as total',
      )
      .leftJoin('user.users as us', 'us.id', 'fb.created_by')
  ...
  public async getSingleFlightBooking(wherePayload: {
    ...
    return await this.db('booking.flight_booking as fb')
  ...
  public async getFlightTraveler(flight_booking_id: number) {
    return await this.db("booking.flight_booking_traveler as fb")
      .select("fb.*")
      .where({ flight_booking_id });
  }
  ...
  public async insertFlightSegment(payload: IInsertFlightSegmentPayload | IInsertFlightSegmentPayload[]) {
    return await this.db('flight_segment')
      .withSchema(this.BOOKING_SCHEMA)
      .insert(payload);
  }
  // insert flight traveler
  public async insertFlightTraveler(payload: IInsertFlightTravelerPayload | IInsertFlightTravelerPayload[]) {
  ...
  //update booking
  public async updateBooking(payload: { status?: string, cancelled_by?: number }, id: number) {
    return await this.db("flight_booking")
      .withSchema(this.BOOKING_SCHEMA)
      .update(payload)
      .where({ id });
