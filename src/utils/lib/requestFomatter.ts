...
export default class RequestFormatter {
  ...
  public async flightSearch(body: IFlightSearchReqBody) {
  ...
  public revalidateReqBodyFormatter = (
    ...
    const OriginDestinationInformation =
      reqBody.OriginDestinationInformation.map((item, index) => {
        ...
        const utils = new FlightUtils();
        if (req_depart_air === depart_air) {
          for (const option of flight.options) {
            const DepartureDateTime = utils.convertDateTime(
              option.departure.date,
              option.departure.time
            );
            const ArrivalDateTime = utils.convertDateTime(
              option.arrival.date,
              option.arrival.time
            );
          ...
          const origin_destination_info = {
            RPH: item.RPH,
            DepartureDateTime: utils.convertDateTime(
  ...
  public newRevalidateReqBodyFormatterV2 = (
    reqBody: IFlightRevalidateReqBody
  ) => {
    const OriginDestinationInformation =
      reqBody.OriginDestinationInformation.map((item) => {
        const flights: any[] = [];
  ...
  //cancel booking
  public cancelBookingReqBody = (pnrId: string) => {
    return {
