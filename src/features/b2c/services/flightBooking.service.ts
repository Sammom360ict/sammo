        ...
        return await this.db.transaction(async (trx) => {
            ...
            const traveler_promises = body.passengers.map(async (element: any) => {
                if (element.save_information) {
                    const { mid_name, save_information, country, passport_expire_date, reference, phone, ...rest } = element;
                    return travelersModel.insertTraveler({
                        ...
                        passport_expiry_date: passport_expire_date ? passport_expire_date.split("T")[0] : undefined,
                        ...rest,
                    });
                }
            });
