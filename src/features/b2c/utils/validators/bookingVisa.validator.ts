...
export class VisaValidator {
    ...
    private traveler_types = ['ADT', 'INF', 'C02', 'C03', 'C04', 'C05', 'C06', 'C07', 'C08', 'C09', 'C10', 'C11'];
    ...
    private travelerSchema = Joi.object({
        type: Joi.string().valid(...this.traveler_types).required(),
        title: Joi.string().valid(...this.traveler_titles).required(),
    ...
    public applicationSchema = Joi.object({
        ...
        passengers: Joi.array().items(this.travelerSchema.required()).required(),
    });
