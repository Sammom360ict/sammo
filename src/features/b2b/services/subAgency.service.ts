...
export class BtoBSubAgencyService extends AbstractServices {
    ...
    public async get(req: Request) {
        ...
        const { data, total } = await agencyModel.getAgency(
            { ref_id: agency_id, name:name as string, status: status as string, limit: limit as unknown as number, skip: skip as unknown as number }
        );
    ...
    public async getSingle(req: Request) {
        ...
        const users = await agencyModel.getUser({ agency_id: Number(id), limit: limit as unknown as number, skip: skip as unknown as number });
