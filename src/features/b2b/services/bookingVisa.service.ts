...
export class B2BVisaService extends AbstractServices {
    ...
    public async createVisaApplication(req: Request) {
        return await this.db.transaction((async (trx) => {
            ...
            const create_application = await model.b2bCreateApplication(application_body);
    ...
    public async getApplicationList(req: Request) {
        ...
        const data = await model.getB2BApplication({agent_id:id,limit:limit?Number(limit):100,skip:skip?Number(skip):0},true);
        return{
            success:true,
