...
class AdministrationService extends AbstractServices {
    ...
    public async createRole(req: Request) {
        ...
        const create_role = await model.createRole({ name: role_name, created_by: id });
    ...
    public async createPermission(req: Request) {
        const { id } = req.admin;
        const model = this.Model.administrationModel();
        const check_name = await model.permissionsList({ name: req.body.permission_name });
        ...
        const create_permission = await model.createPermission({ name: req.body.permission_name, created_by: id });
    ...
    public async permissionList(req: Request) {
        const { limit, skip } = req.query;
        const model = this.Model.administrationModel();
        const permission_list = await model.permissionsList({ limit: Number(limit), skip: Number(skip) }, true);
        return {
            success: true,
            code: this.StatusCode.HTTP_OK,
        ...
        return await this.db.transaction(async (trx) => {
            ...
            if (add_permissions && add_permissions.length) {
                for (const add_permission of add_permissions) {
                    const check_role_permission = await model.getRolePermissions(Number(role_id), Number(add_permission))
                    ...
                    await model.createRolePermission({ role_id: Number(role_id), permission_id: add_permission })
                }
            }
            if (remove_permissions && remove_permissions.length) {
                for (const remove_permission of remove_permissions) {
                    const check_role_permission = await model.getRolePermissions(Number(role_id), Number(remove_permission))
                    if (!check_role_permission.length) {
                        await trx.rollback({
                            success: false,
                    ...
                    await model.deleteRolePermission({ role_id: Number(role_id), permission_id: remove_permission })
                }
            }
            return {
                success: true,
    ...
    public async createAdmin(req: Request) {
        ...
        const check_admin = await model.getSingleAdmin({ email, phone_number, username });
    ...
    public async updateAdmin(req: Request) {
        ...
        if (req.body.phone_number) {
            const check_phone = await model.getSingleAdmin({ phone_number: req.body.phone_number });
    ...
    public async createCity(req: Request) {
        ...
        const data = await model.getAllCity(req.body.country_id, 1, 0, undefined, req.body.name);
        if (data.length) {
            return {
                success: true,
                code: this.StatusCode.HTTP_OK,
